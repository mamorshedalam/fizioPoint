...
export default function Faq({ title, pass }) {
     ...
     return (
          <li className="wow fadeInDown border-y border-black/15 overflow-hidden">
               <button onClick={() => { setActive(!active) }} className={`w-full flex items-center justify-between md:py-5 py-3 md:px-6 px-5 ${active ? 'bg-green-20' : 'bg-green-20/0'}`}>
               ...
               <p className={`${active ? 'max-h-96 md:py-5 py-3' : 'max-h-0'} md:px-6 px-5`}>{pass}</p>
          </li>
     )
}